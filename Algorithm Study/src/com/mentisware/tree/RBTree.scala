...
abstract class RBTree[T : Ordering, Elem] extends SearchTree[T, Elem] {
  def elem: Elem
  ...
  def insert(n: Elem): RBTree[T, Elem]
  def delete(n: Elem): RBTree[T, Elem]
...
class NilTree[T : Ordering, E <: Node[T]] extends RBTree[T, E] {
  def color = Black
  def isEmpty = true
  def size = 0
  ...
  def rankIn(mainTree: RBTree[T, E]) = error("NilTree.rankIn")
  
  def minimum = None
  def maximum = None
  def insert(n: E) = error("NilTree.insert")
  def delete(n: E) = error("NilTree.delete")
  
  override def toString = ""
}

class NormalTree[T : Ordering, E <: Node[T]](
    val elem: E,
    val left: RBTree[T, E],
    val right: RBTree[T, E],
    val color: Color,
    private val nil: NilTree[T, E]
    ) extends RBTree[T, E] {
  def isEmpty = false
  def key = elem.key
  ...
  protected def createNode(e: E, l: RBTree[T, E], r: RBTree[T, E], c: Color): RBTree[T, E] = new NormalTree(e, l, r, c, nil)
  ...
  def predecessor = left match {
    case node: NormalTree[_, _] => node.findMaximum()
    case _ => nil
  }
  
  def successor = right match {
    case node: NormalTree[_, _] => node.findMinimum()
    case _ => nil
  }
  ...
  def rankIn(mainTree: RBTree[T, E]): Int = {
    def rank(m: RBTree[T, E], offset: Int): Int = {
      if (key == m.key) offset + m.left.size + 1
      else if (key < m.key) rank(m.left, offset)
      else rank(m.right, offset + m.left.size + 1)
    }
    
    rank(mainTree.asInstanceOf[RBTree[T, E]], 0)
  }
...
object RBTree {
  def build[T : Ordering, E <: Node[T]](xs: List[E]): RBTree[T, E] = {
    ...
    if (xs.length == 0) nil
      ...
      val tree: RBTree[T, E] = new NormalTree[T, E](xs.head, nil, nil, Black, nil)
